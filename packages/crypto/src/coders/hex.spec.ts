  ...
  test("hex -> byte -> hex", () => {
    fc.assert(
      fc.property(paddedHexArb, (hexString) => {
        expect(hex.encode(hex.decode(hexString))).to.be.equal(
...
describe("Hex <-> Bytes serialization", () => {
  it("Should convert hex to bytes", () => {
    expect(hex.decode("deadbeef")).to.be.deep.equal(
      ui8a([0xde, 0xad, 0xbe, 0xef])
    );
    expect(hex.decode("DEADBEEF")).to.be.deep.equal(
      ui8a([0xde, 0xad, 0xbe, 0xef])
    );
  ...
  it("Should convert bytes to hex", () => {
    ...
    expect(hex.encode(ui8a([0xca, 0xfe, 0x12, 0x34, 0x56]))).to.be.equal(
      "cafe123456"
    );
  });
  ...
  it("Should roundtrip", () => {
    ...
    expect(
      hex.decode(hex.encode(ui8a([0xca, 0xfe, 0x12, 0x34, 0x56])))
    ).to.be.deep.equal(ui8a([0xca, 0xfe, 0x12, 0x34, 0x56]));
  });

  test("Hex to byte with invalid inputs", () => {
    expect(() => hex.decode("non hex string")).to.throw(
