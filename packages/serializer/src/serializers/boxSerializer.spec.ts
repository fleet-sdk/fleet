...
describe("Serialize ErgoBox", () => {
  ...
  it("Should fail with BoxCandidate without distinct tokens list", () => {
    expect(() => {
      serializeBox({
        boxId:
          "8f281813a88e3016d0e0e7b83c5917931f63b610e4dc9af84ad1adecae50778d",
        value: 14996250000n,
  ...
  it("Should estimate the box size in bytes with custom value", () => {
    for (const tv of boxVectors) {
      expect(
        estimateBoxSize(tv.json, ensureBigInt(tv.json.value) * 4n)
      ).toBeGreaterThan(byteSizeOf(tv.hex));
    }
  });
