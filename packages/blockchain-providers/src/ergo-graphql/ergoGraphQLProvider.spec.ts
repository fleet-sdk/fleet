...
describe("ergo-graphql provider", () => {
  ...
  describe("Fetch unspent boxes", () => {
    ...
    it("Should fetch boxes with default params", async () => {
      ...
      const fetchSpy = vi.spyOn(global, "fetch").mockResolvedValueOnce(resolve(mockedData));
  ...
  describe("fetch confirmed transactions", () => {
    ...
    it("Should handle multi-paged queries", async () => {
      ...
      expect(JSON.parse(fetchSpy.mock.calls[0]?.[1]?.body as string).variables).to.be.deep.equal({
        transactionIds: ["txId"],
        skip: 0,
        take: 50
      });

      expect(JSON.parse(fetchSpy.mock.calls[1]?.[1]?.body as string).variables).to.be.deep.equal({
        transactionIds: ["txId"],
        skip: 50,
        take: 50
      });

      expect(JSON.parse(fetchSpy.mock.calls[2]?.[1]?.body as string).variables).to.be.deep.equal({
        transactionIds: ["txId"],
        skip: 100,
        take: 50
      });
  ...
  it("Should fetch LastHeaders with default params", async () => {
    ...
    const fetchSpy = vi.spyOn(global, "fetch").mockResolvedValueOnce(resolveString(mockData));
