...
describe("Keyed mock chain party", () => {
  ...
  it("Should create a non-keyed party", () => {
    const nonKeyedNamedParty = new NonKeyedMockChainParty(chain, FEE_CONTRACT, "miner");
    ...
    const nonKeyedUnnamedParty = new NonKeyedMockChainParty(chain, FEE_CONTRACT);
  ...
  it("Should throw if creating a non-keyed party without an ErgoTree", () => {
    expect(() => new NonKeyedMockChainParty(chain, {} as unknown as string)).to.throw(
